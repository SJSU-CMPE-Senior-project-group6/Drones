...
//Specification:
    //motor: 
oled.printf("Creating Pwm Motor:\n Pwm1,2,3,5");
//Pwm1:2_0 		Pwm2:2_1	Pwm3:2_2	Pwm5:2_4	
  ...
  uint32_t frequency = 492;//need to look at your own motor and ESC data sheet
                           //hardware config: 
                            //ESC: Hobbywing PlatinumPro 30A, Brushless Motor: Proporive 28-30s 900kv 
  float duty = 0.47;//0.47 offset, will be different by brand of motor, need to look at the data sheet
...
//Test first PWM output
  while (true)
  {
      ...
      if(button0.Pressed()){//speed up
      	leds.Toggle(0);
      	duty = duty + 0.05;
      	if(duty > 1){ //maximum no more than 1
      		duty = 0.99;
      	}
      }
      else if(button1.Pressed()){ //slow down 
      	leds.Toggle(1);
      	duty = duty - 0.05;
      	if(duty < 0.47){ // no less than offset
      		duty = 0.47;
      	}
      }
      else if(button2.Pressed()) { //reset to offset
      	leds.Toggle(2);
      	duty = 0.47;
      }
      else{
      ...
      //// uncomment to setup other three motor
